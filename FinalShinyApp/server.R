...
shinyServer(function(input, output) {
  ...
  output$graphType <- renderUI({
    selectedVar <-
      ifelse(is.null(input$edaFeature), "am", input$edaFeature)
  ...
  output$edaPlot <- renderPlot({
    selectedGraphType <-
      ifelse(is.null(input$graphType), "Violin", input$graphType)
    ...
    edaplot <-
      ggplot(data = dataMtcars, aes_string(y = "mpg", x = xAxis, fill = xAxis))
  ...
  model <- reactive({
    fit <- NULL
    dataMtcars <- dataset()
    selectedVars <- input$chkModelFeature
    selectedAutomationType <- input$fitAutomation
    
    if (selectedAutomationType == "Manual") {
      factors <- if (is.null(selectedVars)) {
        c("am")
      } else {
        selectedVars
      }
    ...
    else if (selectedAutomationType == "Automated") {
      fit = step(lm(mpg ~ ., data = dataMtcars),
                 trace = 0,
                 steps = 100)
    }
